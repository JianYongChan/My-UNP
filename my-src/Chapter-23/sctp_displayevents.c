...
print_notification(char *notify_buf)
{
    ...
    switch(snp->sn_header.sn_type) {
        case SCTP_ASSOC_CHANGE:
            ...
            printf("SCTP_ASSOC_CHANGE: %s, assoc_id=0x%x\n",
        ...
        case SCTP_PEER_ADDR_CHANGE:
            ...
            printf("SCTP_PEER_ADDR_CHANGE: %s, addr=%s, assoc_id=0x%x\n",
                    str, sock_ntop((SA *)&spc->spc_aaddr, sizeof(spc->spc_aaddr)), (uint32_t)spc->spc_assoc_id);
            break;
        case SCTP_REMOTE_ERROR:
            sre = &snp->sn_remote_error;
            printf("SCTP_REMOTE_ERROR: assoc_id=0x%x, error=%d\n",
                    (uint32_t)sre->sre_assoc_id, sre->sre_error);
            break;
        case SCTP_SEND_FAILED:
            ssf = &snp->sn_send_failed;
            printf("SCTP_SEND_FAILED: assoc_id=0x%x, error=%d\n",
                    (uint32_t)ssf->ssf_assoc_id, ssf->ssf_error);
            break;
        case SCTP_ADAPTATION_INDICATION:
            sae = &snp->sn_adaptation_event;
            printf("SCTP_ADAPTATION_INDICATION: 0x%x\n",
                    (uint32_t)sae->sai_adaptation_ind);
            break;
        case SCTP_PARTIAL_DELIVERY_EVENT:
            spe = &snp->sn_pdapi_event;
            if (spe->pdapi_indication == SCTP_PARTIAL_DELIVERY_ABORTED)
                printf("SCTP_PARTIAL_DELIVERY_ABORTED\n");
            else
                printf("Unknown SCTP_PARTIAL_DELIVERY_EVENT 0x%x\n",
                        spe->pdapi_indication);
            break;
